...
#include "SideSetHeatTransferKernel1.h"

registerMooseObject("jvdpApp", SideSetHeatTransferKernel1);

template <>
InputParameters
validParams<SideSetHeatTransferKernel1>()
{
  ...
  params.addParam<MaterialPropertyName>(
      ...
      "This value contains contributions from reflectivity, see SideSetHeatTransferMaterial "
      "for details.");
  params.addParam<int>("IsDebug",0,"0->no print;1->print");

  return params;
}

SideSetHeatTransferKernel1::SideSetHeatTransferKernel1(const InputParameters & parameters)
...
Real
SideSetHeatTransferKernel1::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;

  if (_cond[_qp] != 0.0 && _IsDebug == 1 )
...
Real
SideSetHeatTransferKernel1::computeQpJacobian(Moose::DGJacobianType type)
{
  Real jac = 0;
